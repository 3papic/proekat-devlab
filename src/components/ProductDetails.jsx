...
const ProductDetails = () => {
  ...
  const { image, title, description, category, brand, rating, price } = product;

  const slicedRating = Array.isArray(rating) ? rating : "";
    ...
    <section className="flex flex-col gap-16 py-10 bg-gray-100">
      <div className="container mx-auto flex justify-around items-center w-[80%]">
        <div className="w-96 flex justify-end">
          <img src={image} alt={title} className="w-full select-none" />
        </div>
          ...
          <h3 className="flex justify-between text-gray-700 text-lg">
            ...
            <span>
              ...
              <span className="text-rose-500 font-bold">
                {Array.isArray(slicedRating) ? slicedRating.slice(0, 3) : ""}
              </span>
              <span>{Array.isArray(slicedRating) ? slicedRating.slice(3) : ""}</span>
            </span>
          </h3>
          <button
